...
module.exports = async function handler(req, res) {
  ...
  const hace2 = new Date(today); hace2.setDate(hace2.getDate() - 2);
  const pad = n => String(n).padStart(2, '0');
  const fecha2 = `${hace2.getFullYear()}-${pad(hace2.getMonth()+1)}-${pad(hace2.getDate())}`;
  const fecha2Bmx = `${pad(hace2.getDate())}/${pad(hace2.getMonth()+1)}/${hace2.getFullYear()}`;
  ...
  const path = `/SieAPIRest/service/v1/series/SF43718/datos/${fechaDesde}/${fecha2}?token=${TOKEN}`;
